...
class TestShoppingBasket(unittest.TestCase):
    ...
    def test_check_single_item_is_in_shopping_basket(self):
        self.shopping_basket.items.append(Item('book', 12.49, 1))
        self.assertIn(Item('book', 12.49, 1), self.shopping_basket.items)

    def test_check_items_are_in_shopping_basket(self):
        self.shopping_basket.items.append(Item('book', 12.49, 1))
        self.shopping_basket.items.append(Item('music CD', 16.49, 1))

        self.assertIn(Item('book', 12.49, 1), self.shopping_basket.items)
        self.assertIn(Item('music CD', 16.49, 1), self.shopping_basket.items)
